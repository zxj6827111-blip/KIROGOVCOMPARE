/**
 * 测试 Mock 后端的资产管理和 PDF 解析功能
 */

import axios from 'axios';
import * as path from 'path';
import * as fs from 'fs';

const API_BASE_URL = 'http://localhost:3000/api/v1';

async function testMockBackend() {
  console.log('🧪 开始测试 Mock 后端...\n');

  try {
    // 1. 检查健康状态
    console.log('1️⃣  检查后端健康状态...');
    const healthRes = await axios.get('http://localhost:3000/health');
    console.log(`✅ 后端状态: ${healthRes.data.status} (${healthRes.data.mode})\n`);

    // 2. 获取初始资产列表
    console.log('2️⃣  获取初始资产列表...');
    const assetsRes1 = await axios.get(`${API_BASE_URL}/assets`);
    console.log(`✅ 初始资产数: ${assetsRes1.data.total}\n`);

    // 3. 上传资产
    console.log('3️⃣  上传资产...');
    const uploadRes = await axios.post(`${API_BASE_URL}/assets/upload`, {
      fileName: '测试年报.pdf',
      year: 2023,
      region: 'huangpu_city',
      fileSize: 1024000,
    });
    const assetId = uploadRes.data.assetId;
    console.log(`✅ 资产已上传: ${assetId}\n`);

    // 4. 获取资产详情
    console.log('4️⃣  获取资产详情...');
    const assetRes = await axios.get(`${API_BASE_URL}/assets/${assetId}`);
    console.log(`✅ 资产详情:`);
    console.log(`   - 文件名: ${assetRes.data.fileName}`);
    console.log(`   - 年份: ${assetRes.data.year}`);
    console.log(`   - 地区: ${assetRes.data.region}`);
    console.log(`   - 状态: ${assetRes.data.status}\n`);

    // 5. 获取资产列表（验证上传的资产是否在列表中）
    console.log('5️⃣  获取更新后的资产列表...');
    const assetsRes2 = await axios.get(`${API_BASE_URL}/assets`);
    console.log(`✅ 当前资产数: ${assetsRes2.data.total}`);
    console.log(`   资产列表:`);
    for (const asset of assetsRes2.data.assets) {
      console.log(`   - ${asset.assetId}: ${asset.fileName} (${asset.year}年, ${asset.region})`);
    }
    console.log();

    // 6. 获取资产内容（测试 PDF 解析）
    console.log('6️⃣  获取资产内容（测试 PDF 解析）...');
    try {
      const contentRes = await axios.get(`${API_BASE_URL}/assets/${assetId}/content`);
      console.log(`✅ 资产内容已获取:`);
      console.log(`   - 文件名: ${contentRes.data.fileName}`);
      console.log(`   - 年份: ${contentRes.data.year}`);
      console.log(`   - 章节数: ${contentRes.data.parsedContent.sections.length}`);
      
      if (contentRes.data.parsedContent.sections.length > 0) {
        console.log(`   - 章节列表:`);
        for (let i = 0; i < contentRes.data.parsedContent.sections.length; i++) {
          const section = contentRes.data.parsedContent.sections[i];
          console.log(`     ${i + 1}. ${section.title}`);
          console.log(`        - 表格数: ${section.tables ? section.tables.length : 0}`);
        }
      }
    } catch (error: any) {
      if (error.response?.status === 404) {
        console.log(`⚠️  资产内容未找到（可能是 PDF 文件不存在）`);
      } else {
        throw error;
      }
    }
    console.log();

    console.log('='.repeat(60));
    console.log('✅ 所有测试完成！');

  } catch (error: any) {
    console.error('❌ 测试失败:', error.message);
    if (error.response?.data) {
      console.error('   响应:', error.response.data);
    }
    process.exit(1);
  }
}

// 等待后端启动
async function waitForBackend(maxRetries = 30) {
  for (let i = 0; i < maxRetries; i++) {
    try {
      await axios.get('http://localhost:3000/health', { timeout: 1000 });
      return;
    } catch (error) {
      if (i < maxRetries - 1) {
        console.log(`⏳ 等待后端启动... (${i + 1}/${maxRetries})`);
        await new Promise(resolve => setTimeout(resolve, 1000));
      }
    }
  }
  throw new Error('后端启动超时');
}

async function main() {
  console.log('🚀 启动 Mock 后端测试...\n');
  
  try {
    await waitForBackend();
    await testMockBackend();
  } catch (error) {
    console.error('❌ 错误:', error);
    process.exit(1);
  }
}

main();
