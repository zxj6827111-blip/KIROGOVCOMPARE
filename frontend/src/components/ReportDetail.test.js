...
describe('ReportDetail vision review integration', () => {
  ...
  test('detail tab badges count only pending FAIL or UNCERTAIN items', async () => {
    apiClient.get.mockImplementation((url) => {
      switch (url) {
        case '/reports/123':
          return Promise.resolve({ data: { data: reportPayload } });
    ...
    await waitFor(() => {
      expect(screen.getByRole('button', { name: /勾稽关系校验\s*1/ })).toBeInTheDocument();
      expect(screen.getByRole('button', { name: /^数据质量审计$/ })).toBeInTheDocument();
    });
  });
