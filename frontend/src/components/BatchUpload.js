...
function BatchUpload({
      ...
      (async () => {
        ...
        setFiles((prev) =>
          prev.map((f) => {
            ...
            return res
              ? { ...f, duplicate: res.exists, duplicateEmpty: res.exists && !res.hasContent }
              : f;
          })
        );
      })();
  ...
  const updateFile = (id, updates) => {
    setFiles((prev) =>
      prev.map((f) =>
        f.id === id
          ? { ...f, ...updates, matchStatus: updates.regionId ? 'manual' : f.matchStatus }
          : f
      )
    );
  };
      ...
      try {
        ...
        if (jobId && versionId) {
          ...
          setFiles((prev) =>
            prev.map((f) =>
              f.id === fileItem.id
                ? { ...f, status: 'parsing', message: '已提交，后台解析中...' }
                : f
            )
          );
  ...
  const getStatusIcon = (status) => {
    switch (status) {
      ...
      case 'uploading':
        return <Loader size={18} className="spin text-primary" />;
      case 'parsing':
        return <Loader size={18} className="spin text-warning" />;
      case 'success':
        return <Check size={18} className="text-success" />;
      case 'failed':
        return <XCircle size={18} className="text-danger" />;
      case 'skipped':
        return <SkipForward size={18} className="text-muted" />;
      default:
        return <AlertTriangle size={18} className="text-muted" />;
    }
  };
      ...
      <div className="batch-upload-content">
        {!isEmbedded && (
          <div className="batch-upload-header">
            <h2>
              <UploadCloud size={24} style={{ marginRight: '10px' }} /> 批量上传报告
            </h2>
        ...
        {files.length > 0 && (
          <div className="file-list-section">
            <div className="file-list-header">
              <h3>
                <FileText size={18} style={{ marginRight: '8px' }} /> 文件列表 ({files.length}个)
              </h3>
              {stats.unmatched > 0 && (
                    ...
                    {fileItem.duplicate && (
                      <span
                        ...
                        style={{
                          ...
                          borderRadius: '4px',
                          fontSize: '12px',
                          border: fileItem.duplicateEmpty
                            ? '1px solid #fde2e2'
                            : '1px solid #faecd8',
                        }}
                      >
                        {fileItem.duplicateEmpty ? (
                          <span>
                            <XCircle
                              size={12}
                          ...
                          onChange={(e) => {
                            ...
                            checkDuplicate(fileItem.unitName, newYear).then((res) =>
                              updateFile(fileItem.id, {
                                ...
                                duplicateEmpty: res.exists && !res.hasContent,
                              })
                            );
                          }}
                        />
                      ...
                      <span className={`file-region ${!fileItem.regionId ? 'missing' : ''}`}>
                        {fileItem.regionId ? (
                          ...
                          <span className="flex-center">
                            <AlertTriangle size={12} style={{ marginRight: 4 }} /> 未分配
                          </span>
                        )}
                      </span>
                      {fileItem.message && <span className="file-message">{fileItem.message}</span>}
                    </div>
                  )}
                    ...
                    {fileItem.status === 'pending' && !isProcessing && (
                      <>
                        <button
                          className="btn-icon"
                          onClick={() =>
                            setEditingId(editingId === fileItem.id ? null : fileItem.id)
                          }
                          title="编辑"
                        >
                          <Edit2 size={16} />
        ...
        {files.length > 0 && !isProcessing && progress.completed > 0 && (
          <div className="stats-section">
            <span className="stat success">
              <Check size={14} /> 成功: {stats.success}
            </span>
            <span className="stat failed">
              <XCircle size={14} /> 失败: {stats.failed}
            </span>
            <span className="stat skipped">
              <SkipForward size={14} /> 跳过: {stats.skipped}
            </span>
          </div>
        )}
        ...
        <div className="batch-actions">
          {!isProcessing ? (
            <>
              ...
              >
                <Play size={16} fill="currentColor" style={{ marginRight: '8px' }} /> 开始批量上传 (
                {stats.pending}个)
              </button>
            </>
          ) : (
