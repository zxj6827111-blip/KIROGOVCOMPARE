...
export const ReportGenerator: React.FC = () => {
  ...
  const [modelConfig, setModelConfig] = useState({
    deployment: 'local',
    model: 'gemini/gemini-2.5-pro', // 智能辅策默认使用更强模型
    thinkingBudget: 0,
  });
  ...
  const getModelDisplayName = (modelId: string): string => {
    const modelNames: Record<string, string> = {
      ...
      'gemini/gemini-2.5-pro': 'GEMINI3PRO',
    };
  ...
  const prev =
    entity?.data?.find((d) => d.year === year - 1) ||
    ({
      ...current,
      year: year - 1,
      applications: {
      ...
      disputes: {
        reconsideration: { total: 0, corrected: 0 },
        litigation: { total: 0, corrected: 0 },
      },
    } as any);
  ...
  const generateReport = async () => {
    ...
    // Backend timeout is handled there, but we can set a client timeout too
    ...
    try {
      ...
      const prompt = `
        ...
        **JSON Output Mapping (MANDATORY):**
        - **summary**: 
            ...
            3. 复议案量增速：[数值]% （阈值：50%）
            4. 程序性逾期：[数值]件 （阈值：0件）
            依据：[触发了哪条规则，简述原因，例如“纠错率超过30%触发红色预警”]
        ...
        - **critique**:
            - **strengths**: [String Array] 审计发现的亮点（不超过3条，如有数据必须引用，数据需精准）。
            - **weaknesses**: [String Array] 审计发现的风险点与不足（严厉、直接，不留情面。重点关注：程序性逾期、纠错败诉率高、申请渠道不畅等）。

        - **futurePlan**:
            - Array of objects: { title: "String", content: "String" }
            - 针对发现的问题，提出不少于3条具体的、可操作的工作建议。建议应具有前瞻性和针对性。
      ...
      const executeBackendGeneration = async () => {
        ...
        const res = await apiClient.post('/ai/generate-report', {
          ...
          config: {
            ...
            responseSchema: responseSchema, // Passed for reference, backend might use it differently per provider
      ...
      const response = (await Promise.race([
        executeBackendGeneration(),
        timeoutPromise,
      ])) as Partial<GeminiReportResponse>;
      const isValidResponse =
        typeof response?.summary === 'string' &&
        Array.isArray(response?.critique?.strengths) &&
        Array.isArray(response?.critique?.weaknesses) &&
        Array.isArray(response?.futurePlan);

      if (!isValidResponse) {
        throw new Error('AI 返回的 JSON 结构无效，请重试或切换模型');
      }

      setReportData(response as GeminiReportResponse);
      ...
      if (error.message === 'TIMEOUT') {
        errorMsg = '请求超时';
        errorDetail = `模型在 ${TIMEOUT_LIMIT / 1000} 秒内未响应。请检查网络是否稳定，或切换至 'Flash' 模型以获得更快速度。`;
      } else if (error.message.includes('fetch') || error.message.includes('Network')) {
        errorMsg = '网络连接失败';
        errorDetail = '无法连接到服务器 API。请确保网络正常。';
      } else if (error.message.includes('503') || error.message.includes('overloaded')) {
        errorMsg = '服务器繁忙 (503)';
        errorDetail = 'AI 服务当前负载过高，请稍后重试。';
      } else {
        ...
        <tbody>
          <tr className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
            ...
            <td className="py-3 text-right font-bold text-slate-900">
              {fmt(current.applications.newReceived)}
            </td>
            <td className="py-3 text-right text-slate-500">
              {prev.applications.newReceived > 0 ? fmt(prev.applications.newReceived) : '-'}
            </td>
            <td
              className={`py-3 text-right font-medium ${current.applications.newReceived > prev.applications.newReceived ? 'text-rose-600' : 'text-emerald-600'} `}
            >
              {diffPct(current.applications.newReceived, prev.applications.newReceived)}
            </td>
            <td className="py-3 text-center">
              <span
                className={`px-2 py-0.5 rounded text-[10px] font-medium border ${current.applications.newReceived > prev.applications.newReceived * 1.2 ? 'bg-rose-50 text-rose-700 border-rose-200' : 'bg-emerald-50 text-emerald-700 border-emerald-200'} `}
              >
                {current.applications.newReceived > prev.applications.newReceived * 1.2
                  ? '压力激增'
                  : '平稳运行'}
              </span>
            </td>
          ...
          <tr className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
            ...
            <td className="py-3 text-right font-bold text-slate-900">
              {(
                (((current.applications.outcomes?.public || 0) +
                  (current.applications.outcomes?.partial || 0)) /
                  (current.applications.totalHandled || 1)) *
                100
              ).toFixed(1)}
              %
            </td>
            <td className="py-3 text-right text-slate-500">
              {prev.applications.totalHandled > 1
                ? (
                    ((prev.applications.outcomes.public + prev.applications.outcomes.partial) /
                      prev.applications.totalHandled) *
                    100
                  ).toFixed(1) + '%'
                : '-'}
            </td>
            <td className="py-3 text-right text-slate-600">
              {diffPct(
                (current.applications.outcomes.public + current.applications.outcomes.partial) /
                  current.applications.totalHandled,
                (prev.applications.outcomes.public + prev.applications.outcomes.partial) /
                  prev.applications.totalHandled
              )}
            </td>
            <td className="py-3 text-center">
              <span className="bg-blue-50 text-blue-700 border border-blue-200 px-2 py-0.5 rounded text-[10px] font-medium">
                稳定
              </span>
            </td>
          </tr>
          <tr>
            ...
            <td className="py-3 text-right font-bold text-slate-900">
              {(
                (((current.disputes?.reconsideration?.corrected || 0) +
                  (current.disputes?.litigation?.corrected || 0)) /
                  (current.disputes?.reconsideration?.total +
                    (current.disputes?.litigation?.total || 0) || 1)) *
                100
              ).toFixed(1)}
              %
            </td>
            <td className="py-3 text-right text-slate-500">
              {prev.applications.totalHandled > 1
                ? (
                    ((prev.disputes.reconsideration.corrected +
                      prev.disputes.litigation.corrected) /
              ...
              {(() => {
                const curr =
                  ((current.disputes.reconsideration.corrected +
                    current.disputes.litigation.corrected) /
                    (current.disputes.reconsideration.total + current.disputes.litigation.total ||
                      1)) *
                  100;
                const last =
                  ((prev.disputes.reconsideration.corrected + prev.disputes.litigation.corrected) /
                    (prev.disputes.reconsideration.total + prev.disputes.litigation.total || 1)) *
                  100;
                const d = curr - last;
            ...
            <td className="py-3 text-center">
              <span className="bg-rose-50 text-rose-700 border border-rose-200 px-2 py-0.5 rounded text-[10px] font-medium">
                需关注
              </span>
            </td>
          </tr>
  ...
  return (
    <div className="flex gap-6 h-full font-sans text-slate-800 relative">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    setModelConfig((prev) => ({ ...prev, model: m }));
                  }}
                >
                  ...
                  <option value="gemini/gemini-2.5-pro">GEMINI3PRO</option>
                </select>
      ...
      <div className="w-72 flex-shrink-0 flex flex-col gap-4 no-print">
        ...
        {/* Model Info Card */}
        <div
          className={`p-4 rounded-lg shadow-md border relative group transition-colors duration-200 ${engine === 'gemini' ? 'bg-indigo-900 border-indigo-700' : 'bg-slate-800 border-slate-700'} `}
        >
          <button
          ...
          <div className="flex items-center mb-3">
            <Cpu
              className={`w-8 h-8 mr-3 ${engine === 'gemini' ? 'text-pink-400' : 'text-indigo-400'} `}
            />
          ...
          <div className="space-y-2 text-xs text-slate-300">
            <div className="flex justify-between items-center">
              <span>当前内核:</span>
              <span
                className={`font-mono font-bold truncate max-w-[130px] ${engine === 'gemini' ? 'text-pink-300' : 'text-indigo-300'} `}
            ...
            {engine === 'gemini' && (
              <div
                ...
                onClick={() =>
                  setModelConfig((prev) => ({
                    ...prev,
                    thinkingBudget: prev.thinkingBudget > 0 ? 0 : 1024,
                  }))
                }
                title="点击切换思维链模式 (CoT Mode)"
              >
                <span className="flex items-center group-hover:text-white transition-colors">
                  思维链 (CoT)
                  <Settings className="w-3 h-3 ml-1 opacity-0 group-hover:opacity-100 text-slate-400" />
                </span>
                <button
                  type="button"
                  className={`${modelConfig.thinkingBudget > 0 ? 'text-emerald-400 bg-emerald-400/10 border-emerald-400/20' : 'text-slate-500 bg-slate-700/50 border-white/5'} 
                  border px-2 py-0.5 rounded flex items-center transition-all hover:bg-white/10 active:scale-95`}
                >
                  <Zap
                    className={`w-3 h-3 mr-1 ${modelConfig.thinkingBudget > 0 ? 'text-emerald-400 fill-current' : 'text-slate-500'}`}
                  />
            ...
            <div className="mt-3 pt-3 border-t border-white/10">
              <label className="text-[10px] text-indigo-300 block mb-1 uppercase tracking-wider font-bold">
                Switch AI Model
              </label>
              <select
                className="w-full bg-black/20 border border-white/10 rounded text-xs text-white p-1.5 focus:ring-1 focus:ring-indigo-400 outline-none hover:bg-black/30 transition-colors"
                value={modelConfig.model}
                onChange={(e) => {
              ...
              >
                <option value="nvidia/deepseek-ai/deepseek-v3.2" className="text-slate-900">
                  DeepSeek V3.2 (NVIDIA)
                </option>
                <option value="nvidia/moonshotai/kimi-k2.5" className="text-slate-900">
                  Kimi k2.5 (NVIDIA)
                </option>
                <option value="gemini/gemini-2.5-pro" className="text-slate-900">
                  GEMINI3PRO
                </option>
              </select>
            </div>
        ...
        <div className="bg-white p-5 rounded-lg shadow-sm border border-slate-200">
          <div className="flex items-center mb-4">
            <div
              className={`p-2 rounded-lg mr-3 shadow-md ${engine === 'gemini' ? 'bg-gradient-to-br from-pink-500 to-indigo-600 shadow-indigo-200' : 'bg-slate-700 shadow-slate-200'} `}
            >
          ...
          <div className="space-y-4">
              ...
              {engine === 'gemini' ? (
                <>
                  <div className="flex items-center text-indigo-700 font-medium">
                    <CheckCircle2 className="w-3 h-3 text-pink-500 mr-1" /> 复杂归因逻辑推演
                  </div>
                  <div className="flex items-center text-indigo-700 font-medium">
                    <CheckCircle2 className="w-3 h-3 text-pink-500 mr-1" /> 政治话语体系润色
                  </div>
                </>
              ) : (
                <>
                  <div className="flex items-center">
                    <CheckCircle2 className="w-3 h-3 text-slate-400 mr-1" /> 基础数据同比分析
                  </div>
                  <div className="flex items-center">
                    <CheckCircle2 className="w-3 h-3 text-slate-400 mr-1" /> 预设规则风险研判
                  </div>
                </>
              )}
            </div>
            ...
            <button
              ...
              className={`w-full py-3 rounded-lg text-sm font-bold text-white transition-all shadow-md active:scale-95 flex items-center justify-center relative overflow-hidden ${
                isGenerating
                  ? 'bg-slate-400 cursor-not-allowed'
                  : engine === 'gemini'
                    ? 'bg-gradient-to-r from-blue-600 to-indigo-600 hover:shadow-lg hover:shadow-indigo-200'
            ...
            >
              {isGenerating ? (
                <>
                  ...
                  <span
                    className={`font-mono py-0.5 rounded text-xs ml-2 min-w-[30px] text-center ${elapsedTime > 30 ? 'bg-red-500/20 text-red-100' : 'bg-black/10'} `}
                  >
        ...
        {reportData && (
          <div className="bg-white p-5 rounded-lg shadow-sm border border-slate-200 flex-1 overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              ...
              {saveStatus === 'saving' && (
                <span className="text-[10px] text-indigo-500 animate-pulse">☁️ 正在保存...</span>
              )}
              {saveStatus === 'saved' && (
                <span className="text-[10px] text-emerald-600">✅ 已云端归档</span>
              )}
              {saveStatus === 'error' && (
                <span className="text-[10px] text-rose-500">❌ 保存失败</span>
              )}
            </div>
            <ul className="space-y-1 text-xs text-slate-600">
              {[
                ...
                <li
                  key={i}
                  className="flex items-center p-2 rounded hover:bg-slate-50 cursor-pointer transition-colors group"
                >
...
`}
            ...
            <div
              id="printable-report"
              className="bg-white w-[210mm] min-h-[297mm] shadow-2xl text-slate-800 leading-relaxed animate-in fade-in zoom-in-95 duration-500 flex flex-col relative group"
            >
              ...
              <div className="min-h-[297mm] p-[25mm] flex flex-col justify-between relative border-b-2 border-slate-100 bg-gradient-to-br from-white to-slate-50/50 print-cover">
                <div className="mt-24 text-center">
                  <h1 className="text-4xl font-extrabold text-slate-900 mb-6 tracking-tight leading-tight">
                    {year}年度政务公开工作
                    <br />
                    <span className={engine === 'gemini' ? 'text-indigo-600' : 'text-slate-600'}>
                      绩效评估与风险研判报告
                    </span>
                  </h1>
                  <div
                    className={`w-24 h-1.5 mx-auto rounded-full mb-8 ${engine === 'gemini' ? 'bg-indigo-600' : 'bg-slate-600'} `}
                  ></div>
              ...
              <div className="p-[25mm] print-content">
                ...
                <section className="mb-12 break-inside-avoid print-section">
                  <div className="flex items-center mb-6">
                    <span
                      className={`flex items-center justify-center w-8 h-8 rounded-lg text-white font-bold mr-3 shadow-md font-mono ${engine === 'gemini' ? 'bg-indigo-600 shadow-indigo-200' : 'bg-slate-700 shadow-slate-200'} `}
                    >
                      01
                    </span>
                ...
                <section className="mb-12 print-section-break">
                  ...
                  <div className="space-y-6">
                    {reportData.futurePlan?.map((plan: any, idx: number) => (
                      <div
                        key={idx}
                        className="flex items-start p-5 bg-white rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow"
                      >
                        <div
                          className={`flex-shrink-0 w-8 h-8 rounded-full font-bold flex items-center justify-center text-lg mr-4 border ${engine === 'gemini' ? 'bg-indigo-50 text-indigo-600 border-indigo-100' : 'bg-slate-50 text-slate-600 border-slate-200'} `}
                        >
                          {idx + 1}
                        </div>
                        <div>
                          <h4 className="text-base font-bold text-slate-800 mb-2">{plan.title}</h4>
                          <p className="text-sm text-slate-600 leading-relaxed">{plan.content}</p>
                        </div>
                      </div>
                    ))}
                  </div>
                        ...
                        {i === 0 && (
                          <table className="w-full text-xs text-center">
                            ...
                            <tbody className="bg-white text-slate-700">
                              <tr>
                                <td className="py-3 border-r border-slate-100">
                                  {current.regulations.published}
                                </td>
                                <td className="py-3 border-r border-slate-100">
                                  {current.regulations.active}
                                </td>
                                <td className="py-3 border-r border-slate-100">
                                  {current.normativeDocuments.published}
                                </td>
                                <td className="py-3 border-r border-slate-100">
                                  {current.normativeDocuments.active}
                                </td>
                                <td className="py-3 border-r border-slate-100">
                                  {fmt(current.adminActions.licensing)}
                                </td>
                          ...
                          <table className="w-full text-xs text-center">
                            ...
                            <tbody className="bg-white text-slate-700">
                              <tr>
                                ...
                                <td className="py-3 border-r border-slate-100">
                                  {current.applications.carriedOver}
                                </td>
                                <td className="py-3 border-r border-slate-100">
                                  {fmt(current.applications.sources.natural)}
                                </td>
                                <td className="py-3 border-r border-slate-100">
                                  {fmt(current.applications.outcomes.public)}
                                </td>
                                <td className="py-3 border-r border-slate-100">
                                  {fmt(current.applications.outcomes.partial)}
                                </td>
                                <td className="py-3 border-r border-slate-100">
                                  {fmt(current.applications.outcomes.unable)}
                                </td>
                                <td className="py-3">
                                  {fmt(current.applications.outcomes.ignore)}
                                </td>
                              </tr>
                            </tbody>
                          </table>
                          ...
                          <table className="w-full text-xs text-center">
                            <thead className="bg-slate-50 text-slate-600 font-semibold border-b border-slate-200">
                              <tr className="border-b border-slate-200">
                                <th
                                  className="py-2 border-r border-slate-200 bg-slate-100/50"
                                  colSpan={4}
                                >
                                  行政复议
                                </th>
                                <th className="py-2 bg-slate-100/50" colSpan={4}>
                                  行政诉讼
                                </th>
                              </tr>
                              <tr>
                            ...
                            <tbody className="bg-white text-slate-700">
                              <tr>
                                ...
                                <td className="py-3 border-r border-slate-100 text-slate-700">
                                  {current.disputes.reconsideration.corrected}
                                </td>
                                <td className="py-3 border-r border-slate-100">
                                  {current.disputes.reconsideration.maintained}
                                </td>
                                <td className="py-3 border-r border-slate-100">
                                  {current.disputes.reconsideration.pending}
                                </td>
                                <td className="py-3 border-r border-slate-100">
                                  {current.disputes.litigation.total}
                                </td>
                                <td className="py-3 border-r border-slate-100 text-slate-700">
                                  {current.disputes.litigation.corrected}
                                </td>
                                <td className="py-3 border-r border-slate-100">
                                  {current.disputes.litigation.maintained}
                                </td>
          ...
          <div className="w-[210mm] min-h-[297mm] flex flex-col items-center justify-center text-slate-400 border border-dashed border-slate-300 rounded-xl bg-slate-50/50">
            ...
            <p className="max-w-md text-center text-sm text-slate-500 leading-relaxed">
              {engine === 'gemini' ? (
                ...
                <>
                  当前模式：
                  <span className="font-semibold text-indigo-600">GovGPT-Pro (Local Rules)</span>
                  <br />
                  基于本地知识库与专家规则引擎，
                  <br />
                  快速生成标准化的制式分析报告。
            ...
            <button
              onClick={handleGenerate}
              disabled={isGenerating}
              className={`m- p- p- tex-hite tex-m fon-old rounde-ull shado-g transitio-ll active: scal-5 flex item-enter ${
                engine === 'gemini'
            ...
            {engine === 'gemini' && (
              <div className="mt-8 flex items-center space-x-4 text-xs text-slate-400">
                <span className="flex items-center">
                  <Zap className="w-3 h-3 mr-1 text-yellow-500 fill-current" /> Thinking Enabled
                </span>
                <span className="flex items-center">
                  <Shield className="w-3 h-3 mr-1 text-emerald-500" /> Enterprise Security
                </span>
              </div>
            )}
          </div>
