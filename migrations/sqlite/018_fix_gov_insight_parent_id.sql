-- GovInsight VIEW Fix: Return raw region_id and stats
-- We remove the complex string formatting for IDs and parents. 
-- The application layer will join this with the 'regions' table to determine hierarchy.

DROP VIEW IF EXISTS gov_open_annual_stats;

CREATE VIEW gov_open_annual_stats AS
SELECT
  'report_' || r.id AS id, -- Unique ID for the row (technical key)
  r.year,
  r.region_id,             -- The crucial link to regions table
  reg.name AS org_name,    -- Convenience column
  reg.level,               -- Pass through level implementation
